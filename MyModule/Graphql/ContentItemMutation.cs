...
public static class CreateEndpoint
{
    ...
    {
        // builder.MapGet("api/Hycontent", () => Results.Ok("Hycontent API is running"))
        // .RequireAuthorization(policy => policy.RequireRole("Contributor", "Administrator"));

        builder.MapGet("api/Hycontent", async (IAntiforgery antiforgery, HttpContext context) =>
            {
                try
                {
                    await antiforgery.ValidateRequestAsync(context);
                    // Token is valid, process the data
                    return Results.Ok("Data processed successfully.");
                }
        ...
        builder.MapPost("api/Hycontent", HandleAsync)
            .RequireAuthorization(policy => policy.RequireRole("Contributor", "Administrator"));
        ...
        bool draft = false)
    {
        try
                ...
                {
                    // Token validation failed
                    return httpContext.ChallengeOrForbid(e.Message);
                }
